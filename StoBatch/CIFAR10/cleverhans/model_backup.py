...
class CustomCallableModelWrapper(Model):
    ...
    def __init__(self, callable_fn, output_layer, perturbH, params, training_switch):
        """
        Wrap a callable function that takes a tensor as input and returns
        a tensor as output with the given layer name.
        :param callable_fn: The callable function taking a tensor and
        ...
        self.output_layer = output_layer
        self.callable_fn = callable_fn
        self.perturbH = perturbH
        self.params = params
        self.training_switch = training_switch

    def fprop(self, x, **kwargs):
        return {self.output_layer: self.callable_fn(x, self.perturbH, self.params, self.training_switch)}
